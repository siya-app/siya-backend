...
export const isTokenValid = (req: Request, res: Response, next: NextFunction) => {
   console.log('--- isTokenValid middleware ---');
  const authHeader = req.headers['authorization'];
console.log('Authorization Header:', authHeader);
  ...
  try {
    ...
    if (!process.env.JWT_SECRET) {
        ...
        return res.status(500).json({ message: 'Error interno del servidor: clave secreta no configurada.' });
    }
    const decoded = jwt.verify(token, process.env.JWT_SECRET); // Debes usar la misma clave que usaste al firmar
    console.log('Decoded Token Payload:', decoded);
    req.user = decoded; // Aquí se guarda el { id, email, ... }
    console.log('req.user después de decodificar en isTokenValid:', req.user);
    next();
  } catch (err) {

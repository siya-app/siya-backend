...
beforeAll(async () => {
    try {
        const fullBusinesses = await readJsonArray("./businesses-restaurants.json");
        const fullTerraces = await readJsonArray("./terraces.json");
...
describe('matchByCoords', () => {
    it('returns null when businesses is not an array', () => {
        const result = matchByCoords(terraces[0], null, 0.000045);
